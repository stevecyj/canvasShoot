...
function () {
  ...
  _createClass(Boss, [{
    ...
    value: function disappear() {
      ...
      var rotateNum = getRandom(45, 225);
      TweenLite.to(this, 1.6, {
        axisRotateAngle: "+=".concat(rotateNum),
        rotate: "+=".concat(rotateNum),
        ease: Expo.easeOut
  ...
  attackShooterResult: function attackShooterResult() {
    ...
    if (shooterHPBarW <= 0) {
      ...
      if (shooter.hearts) {
        ...
        gameCrawler.textContent = "YOU, ".concat(game.playerName, "\uD83D\uDC80, ARE DEAD");
        return;
      }
    }
...
var starWrapper3 = document.getElementById("star-wrapper3"); // const shareFb = document.getElementById("share-fb");
...
function () {
  ...
  _createClass(Game, [{
    ...
    value: function handleGameResult() {
      ...
      axios.get("https://script.google.com/macros/s/AKfycbxXPc6OojwUB7_7KZemuaWcSZWUczYWyykoeRkCoYoLbFWOXzU/exec", {
        params: {
          name: name,
          level: level,
          battery: battery,
    ...
    value: function handleGameStars() {
      axios.get("https://script.google.com/macros/s/AKfycbwHefxRQcRXMfAgHyS9rcOo1OonEP-I_hHV3qb-aMW73KnNJvw/exec").then(function (res) {
        var data = res.data;
    ...
    value: function countupTime() {
      ...
      this.countupTimer = setTimeout(function () {
        ...
        var minutes = Math.floor(_this8.beatBossSeconds / 60);
        var trueSeconds = _this8.beatBossSeconds % 60;
        var TimeSeconds = trueSeconds < 10 ? "0".concat(trueSeconds) : trueSeconds;
        var TimeMinutes = minutes < 10 ? "0".concat(minutes) : minutes;
        gameTime.textContent = "".concat(TimeMinutes, ":").concat(TimeSeconds, "\u201D");
    ...
    value: function setLevel(level) {
      switch (level) {
        case 1:
          this.initLevel("01", 15); // const rotateNum = 360;
          // this.boss = new Boss({
  ...
  _createClass(Prop, [{
    ...
    value: function draw() {
      ...
      if (this.img.complete) {
        ctx.save();
        ctx.translate(originPos(this.axisRotateR, this.axisRotateAngle).x, originPos(this.axisRotateR, this.axisRotateAngle).y); // setTimeout(() => {
      ...
      var countLastTime = function countLastTime() {
        ...
        setTimeout(function () {
          ...
          if (lastTime) {
            ...
            prop.classList.add('op0'); // clearTimeout(propInfoTimer);
          }
        }, 1000);
      };
